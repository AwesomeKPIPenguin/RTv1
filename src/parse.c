...
{
	cam->direct = ft_3_rotate_vector(ft_point3new(1.0, 0.0, 0.0),
									 cam->alpha, cam->beta, cam->gamma);
	cam->vs_start_vec = ft_3_rotate_vector(
			ft_vector3new(ft_point3new(0.0, 0.0, 0.0),
						  ft_point3new(0.0, WIN_HEIGHT / 2.0,
									   -WIN_WIDTH / 2.0)),
			cam->alpha, cam->beta, cam->gamma);
	cam->vs_x_step_vec = ft_3_rotate_vector(
			ft_point3new(0.0, 0.0, 1.0), cam->alpha, cam->beta, cam->gamma);
	cam->vs_y_step_vec = ft_3_rotate_vector(
			ft_point3new(0.0, -1.0, 0.0), cam->alpha, cam->beta, cam->gamma);
	cam->vs_start_point = ft_3_add_vector(cam->vs_start_vec,
										  ft_3_add_vector(cam->origin,
														  ft_3_rotate_vector(
																  ft_point3new(
																		  (WIN_WIDTH /
																		   2.0) /
